...
''';
  ...
  Future<void> _showCurrentStep() async {
    ...
    _checkTimer?.cancel();
    _checkTimer = Timer.periodic(Duration(milliseconds: 500), (timer) async {
      try {
  ...
  Future<void> _highlightElement(OnBoardingStep step) async {
    ...
    (function() {
      ...
      const element = document.querySelector('${step.selector}');
      if (!element) {
        const altElement = document.querySelector('.w3-container.w3-blue');
        if (altElement && altElement.querySelector('h1')) {
          element = altElement.querySelector('h1');
        } else {
          return false;
        }
      }
      ...
      tooltip.innerHTML = `
        ...
        <button class="onboarding-next-btn" onclick="window.flutter_inappwebview.callHandler('nextOnboardingStep')">
          Next
        </button>
